...
class PyAVSTADataset(Dataset):
    def __init__(self, annotations, path_to_videos, existing_keys, fps=30, max_chunk_size=32, retry=10):
        print("Sampling from {} annotations with a temporal context of {} seconds".format(len(annotations), args.context_frames/fps))
        ...
        for i in range(self.retry):
            ...
            if added==len(frame_numbers) or i==(self.retry-1):
                ...
                if len(missing_frames)>0:
                    print(f"WARNING: could not read the following frames from {video_id}:", ", ".join([str(x) for x in missing_frames]))
